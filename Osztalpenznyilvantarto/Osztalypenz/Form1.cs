...
namespace Osztalypenz
{
    public partial class Form1 : Form
    {
        ...
        private void Tranzakcio()//új függvény
        {
            // megadtunk-e minden adatot?
            if (textBox1.Text != "" && textBox2.Text != "")
            {
                try// a pénz egész szám?
                {
                    int penz = int.Parse(textBox2.Text);
                    if (penz > 0)//a pénz pozitív?
                    {
                        // befizetés kezelése
                        if (radioButton1.Checked)
                        {
                            ...
                            listBox1.Items.Add(db.ToString() + ". tranzakció: " + textBox1.Text + " befizetett " + penz.ToString() + " Ft-ot. "+ dateTimePicker1.Value);
                        }
                        // kifizetés kezelése
                        else
                        {
                            if (egyenleg >= penz)
                            {
                                ...
                                egyenleg -= penz;
                                label5.Text = egyenleg.ToString() + " Ft";
                                listBox1.Items.Add(db.ToString() + ". tranzakció: " + penz.ToString() + " Ft-ot kifizettünk " + textBox1.Text + " részére. " + dateTimePicker1.Value);
                            }
                            else
                            {
                    ...
                    else//ha a pénz negatív
                    {
                        MessageBox.Show("Hibás összeg!");
                    }
                }
                catch (FormatException)//ha nem számot adtunk meg
                {
                    MessageBox.Show("Hibás adatbevitel!");
                }
            }
            else//ha valamelyik beviteli mező üres
            {
