...
namespace Prog{
    public static class Futar_linq{
        ...
        public static void Main(String[] args){
            var fuvarLista = File.ReadLines("tavok.txt").Select(k => new Fuvar(k)).ToArray();
            ...
            int readKm = int.Parse(Console.ReadLine());
            Console.WriteLine($"{readKm} km esetén fizetendő: {CalcPrice(readKm)}");
        
            using(StreamWriter output = new StreamWriter("dijazas.txt")){
        ...
        class Fuvar{
            public int nap, sorszam, tavolsag;
